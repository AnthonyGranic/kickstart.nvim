...
--
-- Targets a fixed canonical location (NOTES_REPO_ROOT below) so the keymaps
-- work the same from anywhere:
...
function M.open_today() vim.cmd('edit ' .. vim.fn.fnameescape(ensure_today_note())) end
...
vim.api.nvim_create_user_command('DailyNote', M.open_today, { desc = "Open today's daily note" })
vim.api.nvim_create_user_command('Jot', M.open_jot_window, { desc = "Quick-jot to today's note" })

vim.keymap.set('n', '<leader>d', M.open_today, { desc = "Open today's [D]aily note" })
vim.keymap.set('n', '<leader>j', M.open_jot_window, { desc = '[J]ot to today’s note' })
